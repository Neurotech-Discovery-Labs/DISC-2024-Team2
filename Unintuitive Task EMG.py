...
def calibrate():
    ...
    for i in range(4):
        ...
        while (time.time() - start_time) < mvc_calibration_time:   #WHILE THE CLOCKTIME-THE STARTTIME IS LESS THAN CALIBRATION TIME... THEN READ THE DATA AND UPDATED 
            data = dev.read() * 1e6  #read in data & change into micro volts (from volts)  #READ THE DATA FROM THE DELSYS FOR all sensors
            data[2] = data[4]  # swapping to emg 5 because emg 3 doesnt seem to work well
            if data.shape[0] > 0:  
...
def update_position(sensor0, sensor1, sensor2, sensor3):
    ...
    def normalize_and_scale(sensor_value, min_threshold, max_contraction):
        if sensor_value < min_threshold: 
            return 0  # Ignore signals below the threshold completely
        normalized = (sensor_value - min_threshold) / (max_contraction - min_threshold)
        return max(0, normalized)  # Ensure no negative values

    # Normalize sensor inputs
    movement_right = normalize_and_scale(sensor0, noise_levels[0], max_contractions[0])
    movement_up = normalize_and_scale(sensor1, noise_levels[1], max_contractions[1])
    movement_down = normalize_and_scale(sensor2, noise_levels[2], max_contractions[2])
    movement_left = normalize_and_scale(sensor3, noise_levels[3], max_contractions[3])

    # Apply a hard threshold to prevent drift at rest
    movement_threshold = 0.1  # Adjust as needed (higher = less sensitivity)
    
    if movement_right < movement_threshold:
        movement_right = 0
    if movement_left < movement_threshold:
        movement_left = 0
    if movement_up < movement_threshold:
        movement_up = 0
    if movement_down < movement_threshold:
        movement_down = 0

    # Ensure only the strongest opposing movement applies
    if movement_right > movement_left:
        movement_left = 0
    else:
        movement_right = 0
    ...
    # Apply movement changes only if within bounds
    if not (new_x_position >= canvas_width - moving_circle_radius and movement_right > 0):  
        new_x_position += (amount_movement * movement_right)

    if not (new_x_position <= moving_circle_radius and movement_left > 0):  
        new_x_position -= (amount_movement * movement_left)

    if not (new_y_position <= moving_circle_radius and movement_up > 0):  
        new_y_position -= (amount_movement * movement_up)

    if not (new_y_position >= canvas_height - moving_circle_radius * 2 and movement_down > 0):  
        new_y_position += (amount_movement * movement_down)

    # Fix Horizontal Drift: Lock x-position when moving only vertically
    if movement_up > 0 or movement_down > 0:
        new_x_position = x_position[-1]  
